...
async def run_command(
    command: List[str], timeout_seconds: int, input_text: Optional[str] = None
) -> Tuple[int, str, str]:
    """
    Executes an external command (e.g. ffmpeg, ffprobe, piper) asynchronously
    ...
    Args:
        ...
        input_text: Optional text piped to the process's stdin (UTF-8
            encoded), then stdin is closed. When omitted (the default),
            stdin is left exactly as it was before this parameter
            existed -- every pre-existing caller (FFmpeg/FFprobe, which
            never reads stdin) is unaffected.
...
async def retry_async(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_exception = exc
            logger.warning(
                "Attempt %s/%s failed with error: %s: %s", attempt, max_retries, type(exc).__name__, exc,
                exc_info=True,
            )
...
async def get_media_duration_seconds(
    media_path: str, ffprobe_binary: str = "ffprobe", timeout_seconds: int = 1800
) -> float:
    """
    Reads the duration (in seconds) of a video or audio file using FFprobe.

    Extracted as a shared utility (rather than left as a method on any one
    service) because both `services/video_merger.py` (sizing the dubbed
    audio track against the source video) and `services/audio_processing.py`
    (timeline alignment: measuring a synthesized clip's actual duration)
    need identical behavior.

    Args:
        media_path: Path to the video or audio file to inspect.
        ffprobe_binary: The ffprobe executable to invoke.
        timeout_seconds: Maximum time to allow the probe to run.
    ...
    """
    command = [ffprobe_binary, "-v", "error", "-show_entries", "format=duration", "-of", "json", media_path]

    return_code, stdout, stderr = await run_command(command, timeout_seconds=timeout_seconds)
