...
class DubRequest(BaseModel):
    ...
    voice: Optional[str] = Field(
        default=None,
        description="Engine-specific voice name/ID. Defaults to the language's default voice "
        "(ignored by engines that don't support voice selection, e.g. gTTS).",
    )
    tts_engine: Optional[str] = Field(
        default=None,
        description="TTS engine to use: 'gtts', 'edge', 'azure', 'piper', 'xtts', or 'elevenlabs'. "
        "See GET /tts/engines for live availability. Defaults to the server's DEFAULT_TTS_ENGINE.",
    )
...
class TTSEngineInfo(BaseModel):
    """Describes one registered TTS engine's live availability and capabilities."""
    ...
    fallback_rank: Optional[int] = Field(
        default=None, description="1-based position in FALLBACK_TTS_ENGINES, or null if not in the chain."
    )
