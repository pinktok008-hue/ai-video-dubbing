...
async def start_dubbing(request: DubRequest, background_tasks: BackgroundTasks) -> DubResponse:
    ...
    if request.voice and not Settings.is_voice_valid_for_language(request.target_language, request.voice):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    logger.info(
        "Dubbing started: task_id=%s target_language=%s source_language=%s voice=%s",
        task.task_id,
