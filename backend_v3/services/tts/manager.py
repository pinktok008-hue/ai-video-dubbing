...
class TTSEngineManager:
    ...
    async def synthesize(self, request: TTSRequest, engine_name: Optional[str] = None) -> TTSResponse:
        ...
        for position, name in enumerate(self._attempt_order(primary_name)):
            ...
            logger.warning(
                "TTS engine '%s' unavailable/failed (%s); trying next configured engine. "
                "(language=%s, voice=%s, chain_position=%d)",
                name, errors[name], request.language, request.voice, position,
            )
    ...
    async def _try_engine(
        ...
        try:
            ...
            logger.warning(
                "Engine '%s' failed to initialize: %s: %s", engine_name, type(exc).__name__, exc, exc_info=True
            )
            return None, str(exc)
        except Exception as exc:  # noqa: BLE001 - an engine's initialize() must never crash the manager
            logger.error(
                "Engine '%s' raised an unexpected error during initialization: %s: %s",
                engine_name, type(exc).__name__, exc, exc_info=True,
            )
        ...
        try:
            ...
            logger.error(
                "Engine '%s' raised during generate(): %s: %s (language=%s, voice=%s)",
                engine_name, type(exc).__name__, exc, request.language, request.voice, exc_info=True,
            )
            return None, str(exc)
